...
namespace Moamalat.Services
{
      ...
      //where EName :typeof(string)
    {
        ...
        {
            string entityPluralName = GetEntityPluralName();

            ApiRequest apiRequest = new() { Content = Utilities.Encrypt(Serialize<PaginationResult<TEntity>>(_Pagination)) };
            var response = await _httpClient.PostAsJsonAsync<ApiRequest>($"{ControllerName}/GetPaged{entityPluralName}", apiRequest);
        ...
        private string GetEntityPluralName()
        {
            ...
            //// Check for EntityPluralNameAttribute
            //var pluralNameAttr = type.GetCustomAttribute<EntityPluralNameAttribute>();
            //if (pluralNameAttr != null)
            //{
            //    return pluralNameAttr.PluralName;
            //}

            //// Fall back to automatic pluralization
            //return GetPluralizedEntityName(type.Name);

            var type = typeof(TEntity);

            var pluralNameAttr = type.GetCustomAttribute<EntityPluralNameAttribute>();

            return pluralNameAttr?.PluralName ?? $"{type.Name}s";
        }
